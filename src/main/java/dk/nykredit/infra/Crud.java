...
import dk.nykredit.infra.util.Assert;
...
public class Crud<T extends BaseEntity> implements Serializable {
    ...
    public void Crud(InjectionPoint ip) {
        if (ip != null && ip.getType() != null) {
            try {
                ...
                throw new IllegalArgumentException("provide entity class at injection point eg: @Inject Crud<Entity> car");
            }

        } else {
            throw new IllegalArgumentException(
                    "Provide entity at injection point ex: @Inject Crud<Entity> car");
        }
    }
    ...
    public Crud<T> example(T entity, MatchMode mode, List<String> excludeProperties) {
        ...
        if (Assert.has(excludeProperties)) {
            excludeProperties.forEach(example::excludeProperty);
        }
        getCriteria().add(example);
        return this;
    }
